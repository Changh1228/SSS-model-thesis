...
def cal_task(hit,intensity,rpy,vehicle_pos,normal_vec,mesh_reso,ping):
    ...
    plt.subplot(132)
    plt.title('port')
    plt.scatter(np.arange(len(ping.port.pings)), ping.port.pings, s=2, c='b', marker='.')
    #plt.show()

    plt.subplot(133)
    plt.title('stbd')
    plt.scatter(np.arange(len(ping.stbd.pings)), ping.stbd.pings, s=2, c='b', marker='.')
    plt.grid()
    plt.show()
    result = []
...
for index in meas_list[LAYER]:
    ...
    for i in range(row): # for one singal ping
        ...
        hit = np.array([meas_imgs.sss_waterfall_hits_X[i], #[[x,y,z],[x,y,z]...] drape result with low reso 512
        ...
        ping = xtf_pings[index] # xtf_ping with high reso (14212 each side)
        result = cal_task(hit,intensity,rpy,vehicle_pos,normal_vec,mesh_reso, ping)
